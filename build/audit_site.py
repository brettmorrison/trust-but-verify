...
# Pages allowed to put the notice below the first section heading. Empty, and
# meant to stay that way: the notice is generated by build/notice.py and placed
# immediately under the <h1> on every unvalidated page, so there is nowhere
# else for it to be. It held 40 landing pages until TBV-003, where it sat at
# 93-94% of document depth -- below the three steps and below the helpline
# numbers, which is after the reader has already decided whether to trust the
# page. The list may only ever get shorter.
NOTICE_BELOW_FIRST_H2 = set()
