...
"""
import os, re, sys, shutil, html, datetime
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from notice import notice_html
import markdown
...
UI = {
 "en": dict(strap="Hang up. Look up the number yourself. Wait a day.",
   ...
   listen="Listen to this page:", listenlangs="Also in:",
 ...
 "es": dict(strap="Cuelgue. Busque el n\u00famero usted mismo. Espere un d\u00eda.",
   ...
   listen="Escuche esta p\u00e1gina:", listenlangs="Tambi\u00e9n en:",
 ...
 "vi": dict(strap="C\u00fap m\u00e1y. T\u1ef1 m\u00ecnh tra s\u1ed1. Ch\u1edd m\u1ed9t ng\u00e0y.",
   ...
   listen="Nghe trang n\u00e0y:", listenlangs="C\u0169ng c\u00f3 b\u1eb1ng:",
 ...
 "ru": dict(strap="\u041f\u043e\u043b\u043e\u0436\u0438\u0442\u0435 \u0442\u0440\u0443\u0431\u043a\u0443. \u041d\u0430\u0439\u0434\u0438\u0442\u0435 \u043d\u043e\u043c\u0435\u0440 \u0441\u0430\u043c\u0438. \u041f\u043e\u0434\u043e\u0436\u0434\u0438\u0442\u0435 \u0441\u0443\u0442\u043a\u0438.",
   ...
   listen="\u041f\u0440\u043e\u0441\u043b\u0443\u0448\u0430\u0442\u044c \u044d\u0442\u0443 \u0441\u0442\u0440\u0430\u043d\u0438\u0446\u0443:", listenlangs="\u0422\u0430\u043a\u0436\u0435 \u043d\u0430:",
...
UI["ko"] = dict(strap="전화를 끊으십시오. 번호를 직접 찾으십시오. 하루 기다리십시오.",
  ...
  listen="이 페이지 듣기:", listenlangs="다른 언어로도:",
...
def lang_versions_widget(content_slug, current_lang, pre):
    # For an article that's been translated into multiple languages
    # (currently the 5 scam-type articles in ARTICLE_LANGS), a compact
    # "Also in: English · Español · ..." row next to the audio player so
    # a reader -- or a family member checking a different language's
    # version -- doesn't have to hunt through the header switcher.
    versions = [c for c in ARTICLE_LANGS
                if os.path.exists(os.path.join(CONTENT, c, content_slug + ".md"))]
    if len(versions) < 2:
        return ""
    parts = []
    for code in versions:
        label = html.escape(lang_label(code))
        if code == current_lang:
            parts.append("<strong>%s</strong>" % label)
        else:
            href = pre + (content_slug + "/" if code == "en"
                           else "%s/%s/" % (code, content_slug))
            parts.append('<a href="%s" lang="%s">%s</a>' % (href, code, label))
    return '<span class="audio-langs">%s %s</span>' % (
        html.escape(ui(current_lang, "listenlangs")), " · ".join(parts))
...
/* The unvalidated-translation notice, generated by build/notice.py and placed
   immediately under the h1 on every non-English page. It used to sit at 93% of
   document depth on 40 landing pages, below the three steps and below the
   helpline numbers, which is after the reader has already decided whether to
   trust the page. */
.notice{border:3px solid var(--rule);background:var(--band);
  padding:1rem 1.1rem;margin:1.3rem 0 1.8rem}
.notice p{margin:0 0 .6rem}
.notice p:last-child{margin-bottom:0}
.notice .notice-en{color:var(--muted)}
.notice [dir="ltr"],.shell[dir="rtl"] .notice [dir="ltr"]{text-align:left}
...
def build():
    ...
    for src, meta, body, lang, slug in pages:
        ...
        audio_filename = None
        content_slug = slug.strip("/") if lang == "en" else ""
        ...
        if audio_filename:
            langs_row = lang_versions_widget(content_slug or "home", lang, pre)
            player = (
        ...
        # The unvalidated-translation notice, in the reader's own language
        # wherever one exists, immediately under the h1 -- ahead of the photo
        # and the audio player, because it is the thing that decides whether
        # the rest of the page should be trusted at all. Driven by front
        # matter, so a language that gets validated loses its notice from
        # every page at once instead of being unpicked by hand.
        if lang != "en" and meta.get("validated_by", "").strip().startswith("(none"):
            block = notice_html(lang, lang_label(lang))
            body_html = re.sub(r"(</h1>)", lambda m: m.group(1) + block,
                               body_html, count=1)
