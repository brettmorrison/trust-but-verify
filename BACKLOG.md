...
- **All 5 priority languages' scam articles now on ElevenLabs**
  (2026-08-25 to 2026-08-27): Spanish, Vietnamese, Chinese, Korean,
  Russian — phantom-hacker, tech-support-popup, grandparent-scam,
  government-impersonation, romance-scam, 25 articles total, all
  narrated with the paid voice instead of the free macOS fallback.
  Order after Spanish (fixed as #2 per Brett's standing rule) was
  Vietnamese → Chinese → Korean → Russian, first two picked by raw
  foreign-born population, last two confirmed against the better LEP
  age-65+ metric once Brett asked for it (see the standing rule at the
  top of this file). Vietnamese needed the eleven_turbo_v2_5 model
  override (standard multilingual model doesn't support it at all);
  the other four used the standard model. Real character/credit costs
  per batch, checked against the live dashboard between every single
  one (never chained without a fresh number): Spanish 25,208 chars;
  Vietnamese 26,455 chars (~7,276 credits — Turbo is cheaper per
  character); Chinese 9,117 chars (~5,015 credits — dense script,
  cheap); Korean 14,109 chars (~7,761 credits); Russian 25,319 chars
  (cost not yet independently confirmed against the dashboard as of
  this write-up). Cumulative real usage after Korean: 84,350/131,000
  (46,650 remaining) — Russian's batch will move that further, ask
  Brett for the post-Russian number rather than estimating. A "listen
  in other languages" row now sits inside the audio-player box itself
  on every article translated into more than one language (Brett's
  request, 2026-08-27) — see lang_versions_widget() in build_site.py,
  zero maintenance needed as more languages/articles get added since
  it's driven entirely by which content/<lang>/<slug>.md files exist
  on disk.

## Open
1. Beyond the original 5-language set: any 6th+ language should be
   picked using the LEP age-65+ standing metric (see top of file), not
   foreign-born counts. Also still open regardless of budget: a real
   screen-reader pass (VoiceOver, free, built into macOS) rather than
   just automated/structural checks; the original English-batch
   ElevenLabs key expired around 2026-09-24 — TBVP2 (created
   2026-08-25) is the active key now, same 30-day-ish expiry pattern
   likely applies, renew when it lapses.
