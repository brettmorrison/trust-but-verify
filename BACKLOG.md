...
**Editorial learning loop (added 2026-08-27).** Ported from the pattern
proven on another project's blog pipeline (see
`~/Claude/playbooks/editorial/blog-generation-pipeline.md`, re-verified
against that live codebase before porting). Two files at the repo root:
`EDITORIAL_STYLE_GUIDE.md` (hand-written, human-owned, standing rules —
read this before drafting or translating anything) and
`EDITORIAL_LEARNED_PATTERNS.md` (machine-owned, regenerated by
`build/learn_from_edits.py`, never hand-edit). The learner diffs a
content file's original AI-generated version against a later human-edited
version straight out of git history — no database, no admin UI, no login,
matching this site's zero-infrastructure design — and accumulates
occurrence-counted favored/disfavored phrase patterns per language.
Patterns only surface at 2+ occurrences (one edit is noise). Run
`python3 build/learn_from_edits.py --all` to rebuild from full history, or
`python3 build/learn_from_edits.py <file>` after a single validation pass.
**This activates properly once native-speaker validation actually starts**
— today it has no validated pages to learn from, so its current output is
mostly derived from this session's own English copy-editing history (it
did correctly detect the site's em-dash→comma house style, which is a
real signal). Deliberately NOT built: a web editing UI. That would need a
real auth system with MFA (Brett's call, 2026-08-27) and would add a login
surface to a site that currently has zero server-side attack surface
beyond one email-forwarding function. Revisit only if git-based editing
proves genuinely insufficient for validators.
